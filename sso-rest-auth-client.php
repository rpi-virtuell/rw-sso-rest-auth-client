...
//      add_action( 'set_current_user', array( $this, 'remote_login' ) );
    ...
    public function toggle_rpi_maintenance_mode()
    {
        ...
        if (array_key_exists('_wpnonce', $_GET) && array_key_exists('maintenance', $_GET)) {
    ...
    {
        wp_enqueue_script('template_handling', plugin_dir_url(__FILE__) . '/assets/js/sign_in_redirect.js', array(), '1.0', true);
    }
    ...
    {
        if (array_key_exists('action', $_POST) && $_POST['action'] === 'sso_delete_token' && isset($_POST['user_id'])) {
    ...
    public function add_ip_to_whitelist_table()
    {
        ...
        if (!$this->is_current_ip_whitelisted()) {
            $wpdb->insert($wpdb->base_prefix . 'failed_login_whitelist', array(
                'ip' => $ip,
                'last_login' => time(),
            ), array(
                '%s',
    ...
    public function add_failed_login_attempt($username)
    {
        ...
        $result = $wpdb->insert($wpdb->base_prefix . 'failed_login_log', array(
            'hash' => $hash,
            'ip' => $ip,
            'username' => $username,
            'last_login' => time(),
    ...
    function add_invite_user_user_page()
    {
        ...
        add_users_page('invite_user', __('Invite User', 'rw-sso-client'), 'manage_options', 'invite_user', array(
            $this,
            'init_invite_user_page'
        ), 1);
    }
